...
class EventCompiler {
            ...
            elements.forEach( (final dom.Element element) {
                ...
                datasets[dataset](element,(final dom.Event event) {
                    _invokeFunction(
                        ...
                        getParams(),
                        event,
                        eventCallbacks[getFunctionNameAsString()]);
                });
            });
    ...
    ///
    /// [callback] can be null if there is no callback defined for [function]
    void _invokeFunction(final String function, final List params, final dom.Event event,final Function callback) {
        if(_hasNoEvent(params)) {
            //event.preventDefault();
            //event.stopPropagation();
        }
        if(callback == null) {
            _logger.info("No callback defined for: ${function}(${params})");
            return;
        }

        final _params = _replaceEventInParams(params, event);

        _logger.info("Calling: ${function}(${_params})");
        Function.apply(callback, _params);
    }
